...
// Delegates to the shared runner.BuildDownloadScript with verbose=false for cleaner K8s logs.
func buildK8sDownloadScript(spec runner.DataDownloadSpec) string {
	return runner.BuildDownloadScript(spec, false) // verbose=false for cleaner K8s logs
}
