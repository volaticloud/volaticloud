...
func (_q *StrategyQuery) QueryBots() *BotQuery {
...
func (_q *StrategyQuery) WithBots(opts ...func(*BotQuery)) *StrategyQuery {
	query := (&BotClient{config: _q.config}).Query()
...
func (_q *StrategyQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Strategy, error) {
	...
	if query := _q.withBots; query != nil {
		if err := _q.loadBots(ctx, query, nodes,
			func(n *Strategy) { n.Edges.Bots = []*Bot{} },
			func(n *Strategy, e *Bot) { n.Edges.Bots = append(n.Edges.Bots, e) }); err != nil {
			return nil, err
		}
	}
	...
	for name, query := range _q.withNamedBots {
		if err := _q.loadBots(ctx, query, nodes,
			...
			func(n *Strategy, e *Bot) { n.appendNamedBots(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *StrategyQuery) loadBots(ctx context.Context, query *BotQuery, nodes []*Strategy, init func(*Strategy), assign func(*Strategy, *Bot)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(bot.FieldStrategyID)
	}
	query.Where(predicate.Bot(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(strategy.BotsColumn), fks...))
	}))
...
func (_q *StrategyQuery) loadBacktest(ctx context.Context, query *BacktestQuery, nodes []*Strategy, init func(*Strategy), assign func(*Strategy, *Backtest)) error {
	...
	nodeids := make(map[uuid.UUID][]*Strategy)
	for i := range nodes {
		if nodes[i].strategy_backtest == nil {
			continue
		}
		fk := *nodes[i].strategy_backtest
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "strategy_backtest" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *StrategyQuery) WithNamedBots(name string, opts ...func(*BotQuery)) *StrategyQuery {
	query := (&BotClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
