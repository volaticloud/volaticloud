...
// HyperOpt holds the schema definition for the HyperOpt entity.
type HyperOpt struct {
	ent.Schema
}

// Fields of the HyperOpt.
func (HyperOpt) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int("epochs").
			Positive().
			Comment("Number of optimization epochs"),
		...
		field.Enum("optimization_metric").
			GoType(enum.OptimizationMetric("")).
			Default(string(enum.OptimizationMetricSharpe)).
			Comment("Optimization metric"),
		field.JSON("spaces", []string{}).
			Annotations(entgql.Skip(entgql.SkipAll)).
			Comment("Hyperopt spaces (buy, sell, roi, stoploss, trailing)"),
		field.JSON("results", map[string]interface{}{}).
			Optional().
			Annotations(entgql.Skip(entgql.SkipAll)).
			Comment("Best parameters found"),
		field.JSON("config", map[string]interface{}{}).
			Optional().
			Annotations(entgql.Skip(entgql.SkipAll)).
			Comment("HyperOpt-specific configuration"),
...
// Edges of the HyperOpt.
func (HyperOpt) Edges() []ent.Edge {
	return []ent.Edge{
...
// Annotations of the HyperOpt.
func (HyperOpt) Annotations() []schema.Annotation {
