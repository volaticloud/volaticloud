...
interface EditRunnerDrawerProps {
  open: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export const EditRunnerDrawer = ({ open, onClose, onSuccess, runner }: EditRunnerDrawerProps) => {
  ...
  // Reset form when runner changes or drawer opens
  useEffect(() => {
    if (open) {
      setName(runner.name);
      setType(runner.type);
